...
class MotionCapture:
    ...
    def showFFTResults(self, x_time, y_time, x_frequency, y_frequency, dimension):
...
#        ax[1].plot(x_frequency, y_frequency, color=(1.0, 0.0, 0.0))  # plotting the spectrum in BPM
        ...
        ax[1].bar(x_frequency *60, y_frequency, color=(1.0, 0.0, 0.0), width=chart_bar_width )  # plotting the spectrum in BPM
        ax[1].set_xlabel('Pulse (BMP)')
        ax[1].set_ylabel('|Y(BMP)| ' + dimension )

    def showTimeResults(self, beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive):
        fig, ax = plt.subplots(2, 1)
    ...
    def test_filters(self):
        mp = MotionProcessor()
        fps = 12.0;  # frames/seconf (sample rate)
        ...
        # Add some noise at 3 X the pulse rate and 1/3 amplitue, (comment out to remove noise)
        y = np.sin(pulse_rate_seconds * 2.0 * np.pi * x_time) + .3*np.sin(pulse_rate_seconds*3 * 2.0 * np.pi * x_time)

        x_time, y_time, x_frequency, y_frequency = mp.fft_filter_series( y, fps, 'X')
        self.showFFTResults( x_time, y_time, x_frequency, y_frequency, 'X')

        beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive = mp.time_filter_series(y, fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showTimeResults( beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive)
        plt.show()
...
#                        print("width",w, "height", h)
        ...
        self.showFFTResults(x_time, y_time,  x_frequency, y_frequency, 'Y')

        x_time, y_time,  x_frequency, y_frequency = mp.fft_filter_motion('W', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showFFTResults(x_time, y_time,  x_frequency, y_frequency, 'W')

        x_time, y_time,  x_frequency, y_frequency = mp.fft_filter_motion('H', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showFFTResults(x_time, y_time,  x_frequency, y_frequency, 'H')
        # plt.show()

        beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive = mp.time_filter_motion('X', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showTimeResults(beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive)

        beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive = mp.time_filter_motion('Y', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showTimeResults(beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive)

        beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive = mp.time_filter_motion('W', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showTimeResults(beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive)


        beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive = mp.time_filter_motion('H', clip.fps, LOW_PULSE_BPM, HIGH_PULSE_BPM)
        self.showTimeResults(beats_per_minute, x_time, y_amplitude, y_amplitude_filtered, peaks_positive)
